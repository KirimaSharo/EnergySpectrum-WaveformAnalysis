...
const TCHAR *OPSET[OPTR_NUM] = { L"+",L"-",L"*",L"/",L"^",L"ln",L"log",L"sin",L"cos",L"tan",L"sqr",	L"√",	L"(",	L")",	L"=" };//操作符集
...
double get_next_expression(const TCHAR * Expression,int &index,int &isnum){
	...
	while (index < wcslen(Expression)) {
		...
		if (isnum) {
			...
			if (get_operator_index(str) >= 0) {
				//判断str是不是一个完整的操作符
				//一个操作符取出完成
				ret = get_operator_index(str);
				return ret;
			}
		}
...
BOOL EvaluateExpression(const TCHAR * Expression,double &result,int &error_code){
	...
	while (op != OP_INDEX_EQUAL || OPTR.top() != OP_INDEX_EQUAL || isnum != 0) {
		if (isnum){
			//操作数
			OPND.push(op);
			lastType = isnum;
...
BOOL EvaluateExpressionX(const std::string Expression, double &result, double x, int &error_code)
{
	...
	for (int pos = 0;pos >= 0;)
	{
		...
		str = str.insert(pos, istr);
	}
	str += "=";
	
	TCHAR *Exp = new TCHAR[str.size() + 1];
	_stprintf_s(Exp, str.size() + 1, _T("%S"), str.c_str());
	int ret = EvaluateExpression(Exp, result, error_code);
	delete Exp;
	return ret;
}